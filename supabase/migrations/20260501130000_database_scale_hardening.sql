...
-- - no blocking full-table validations during deploy
-- - future writes get stronger constraints immediately
--
...
-- ---------------------------------------------------------------------------
-- Relationship constraints that were missing from generated relationship types
-- ---------------------------------------------------------------------------

do $$
begin
  if to_regclass('public.request_messages') is not null then
  ...
  if to_regclass('public.request_credit_packs') is not null then
  ...
  if to_regclass('public.sms_send_log') is not null then
  ...
  if to_regclass('public.founding_pro_claims') is not null then
...
-- ---------------------------------------------------------------------------
-- Status/domain constraints for future writes
-- ---------------------------------------------------------------------------

do $$
begin
  if to_regclass('public.captured_media') is not null then
...
-- ---------------------------------------------------------------------------
-- Tenant consistency triggers for duplicated workspace_id columns
-- ---------------------------------------------------------------------------
...
as $$
begin
  perform public.assert_request_workspace_match(
    new.request_id,
    new.workspace_id,
    'request_messages workspace mismatch'
  );
  return new;
end;
$$;
...
create trigger request_messages_consistency_trigger
  before insert or update of request_id, workspace_id
  on public.request_messages
  for each row
  execute function public.enforce_request_message_consistency();
...
as $$
begin
  perform public.assert_submission_workspace_match(
    new.submission_id,
    new.workspace_id,
    'submission_reviews workspace mismatch'
  );
  return new;
end;
$$;
...
create trigger submission_reviews_consistency_trigger
  before insert or update of submission_id, workspace_id
  on public.submission_reviews
  for each row
  execute function public.enforce_submission_review_consistency();
...
begin
  perform public.assert_submission_workspace_match(
    new.submission_id,
    new.workspace_id,
    'internal_notes workspace mismatch'
  );
  return new;
end;
$$;
...
create trigger internal_notes_consistency_trigger
  ...
  for each row
  execute function public.enforce_internal_note_consistency();
...
begin
  perform public.assert_submission_workspace_match(
    new.submission_id,
    new.workspace_id,
    'submission_answers workspace mismatch'
  );
  perform public.assert_request_workspace_match(
    new.request_id,
    new.workspace_id,
    'submission_answers request workspace mismatch'
  );
  perform public.assert_submission_request_match(
    new.submission_id,
    new.request_id,
    'submission_answers submission request mismatch'
  );
  return new;
end;
$$;
...
create trigger submission_answers_consistency_trigger
  before insert or update of submission_id, request_id, workspace_id
  on public.submission_answers
  for each row
  execute function public.enforce_submission_answer_consistency();
...
begin
  perform public.assert_request_workspace_match(
    new.request_id,
    new.workspace_id,
    'sms_send_log workspace mismatch'
  );
  return new;
end;
$$;
...
create index if not exists photo_brief_requests_workspace_status_created_idx
  on public.photo_brief_requests(workspace_id, status, created_at desc);

create index if not exists photo_brief_requests_token_idx
  on public.photo_brief_requests(token);

create index if not exists submissions_workspace_status_created_idx
  on public.submissions(workspace_id, status, created_at desc);

create index if not exists submissions_request_created_idx
  on public.submissions(request_id, created_at desc);

create index if not exists captured_media_submission_created_idx
  on public.captured_media(submission_id, created_at asc);

create index if not exists captured_media_step_idx
  on public.captured_media(step_id)
  where step_id is not null;

create index if not exists ai_check_results_media_created_idx
  on public.ai_check_results(captured_media_id, created_at desc);

create index if not exists extracted_details_submission_idx
  on public.extracted_details(submission_id);

create index if not exists internal_notes_submission_created_idx
  on public.internal_notes(submission_id, created_at desc);

create index if not exists submission_reviews_submission_round_idx
  on public.submission_reviews(submission_id, round desc);

create index if not exists request_messages_request_sent_idx
  on public.request_messages(request_id, sent_at desc);

create index if not exists request_messages_workspace_sent_idx
  on public.request_messages(workspace_id, sent_at desc);

create index if not exists subscriptions_workspace_environment_created_idx
  on public.subscriptions(workspace_id, environment, created_at desc);

create index if not exists request_credit_packs_workspace_status_period_idx
  on public.request_credit_packs(workspace_id, status, period_end);

create index if not exists sms_send_log_workspace_sent_idx
  on public.sms_send_log(workspace_id, sent_at desc);

create index if not exists sms_send_log_request_sent_idx
  on public.sms_send_log(request_id, sent_at desc)
...
create index if not exists photo_guides_workspace_active_idx
  on public.photo_guides(workspace_id, is_active, created_at desc)
  where workspace_id is not null;

create index if not exists guide_steps_guide_order_idx
  on public.guide_steps(guide_id, order_index);

create index if not exists context_questions_guide_order_idx
  on public.context_questions(guide_id, order_index);

-- ---------------------------------------------------------------------------
-- Operational uniqueness safeguards
-- ---------------------------------------------------------------------------

do $$
begin
  if to_regclass('public.request_credit_packs') is not null then
    if not exists (select 1 from pg_indexes where schemaname = 'public' and indexname = 'request_credit_packs_checkout_unique_idx') then
      create unique index request_credit_packs_checkout_unique_idx
        on public.request_credit_packs(stripe_checkout_session_id)
        where stripe_checkout_session_id is not null;
    end if;

    if not exists (select 1 from pg_indexes where schemaname = 'public' and indexname = 'request_credit_packs_payment_intent_unique_idx') then
      create unique index request_credit_packs_payment_intent_unique_idx
  ...
  if to_regclass('public.subscriptions') is not null then
    if not exists (select 1 from pg_indexes where schemaname = 'public' and indexname = 'subscriptions_stripe_subscription_unique_idx') then
  ...
  if to_regclass('public.photo_brief_requests') is not null then
    if not exists (select 1 from pg_indexes where schemaname = 'public' and indexname = 'photo_brief_requests_token_unique_idx') then
      create unique index photo_brief_requests_token_unique_idx
        on public.photo_brief_requests(token);
    end if;
  end if;
end $$;
