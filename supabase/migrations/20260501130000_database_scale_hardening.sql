...
--
-- Additive and production-safe: no table moves, no destructive enum conversions,
-- no unique indexes that can fail on legacy duplicates, and no full-table
-- validation during deploy. Future writes are tightened immediately; existing
-- rows can be audited and validated later.
...
-- duplicate audits are run in production. Promote to partial unique indexes in
-- a later migration after confirming there are no legacy duplicate values.
